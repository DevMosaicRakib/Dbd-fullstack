...
const DropDown = ({setDropDown}) => {
    ...
    return (
      <div className='w-[300px] bg-[#ffff] h-auto' style={{
        ...
        {categories?.map((item) => (
          <div key={item.id} className={`${Styles.normal_flex} relative hover:bg-gray-300 catData`}
            ...
            <h3 className='m-3 cursor-pointer select-none h-full font-[400] 1280px:text-[13px]'>{item.catname}</h3>
             </div>
                  ...
                  <div className='subCategoryItems'>
                    {item.sub_categories.map((subcat) => (
                      <div key={subcat.id} className='flex items-center'>
                        ...
                        text-start font-[400] px-[10px] py-[5px]'
                           onClick={(e) => {
